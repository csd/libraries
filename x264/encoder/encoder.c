...
// TID : SLICES. Persistent Threads . Avoid create and destroy between frames
static int x264_slices_write_thread_loop( x264_t *h );
...
//    }
	if( h->param.i_threads > 1 )
	{
		num_threads = h->param.i_threads + h->param.i_slices - 1;
	}
	else
	{
		num_threads = h->param.i_slices;
	}
    ...
    for( i = 0; i < num_threads; i++ )
    {
        ...
        if( x264_macroblock_cache_init( h->thread[i] ) < 0 )
            return NULL;
		if( h->param.i_slices > 1 )
		{
	        h->thread[i]->b_thread_active = 1;
	        
	        // TID : SLICES. Persistent Threads between Frames
			/* Initialize mutex and condition variable objects */
	  		pthread_mutex_init( &(h->thread[i]->thread_mutex_start), NULL );
	  		pthread_cond_init( &(h->thread[i]->thread_cond_start), NULL );
	  		pthread_mutex_init( &(h->thread[i]->thread_mutex_stop), NULL );
	  		pthread_cond_init( &(h->thread[i]->thread_cond_stop), NULL );
	  		h->thread[i]->b_thread_finished = 0;
	  		h->thread[i]->b_thread_ready = 0;
	  		pthread_create( &(h->thread[i]->thread_handle), NULL, (void*)x264_slices_write_thread_loop, h->thread[i] );
		}
    }
...
static void x264_fdec_filter_row( x264_t *h, int mb_y )
{
    ...
    // TID : SLICES. Throw 1 thread per slice
//    if( h->param.i_threads > 1 && h->fdec->b_kept_as_ref )
    if( ( h->param.i_threads > 1 || h->param.i_slices > 1 ) && h->fdec->b_kept_as_ref )
    {
...
static inline void x264_reference_update( x264_t *h )
{
    ...
    if( !h->fdec->b_kept_as_ref )
    {
    	// TID : SLICES. Throw 1 thread per slice
//        if( h->param.i_threads > 1 )
		if( h->param.i_threads > 1 || h->param.i_slices > 1 )
        {
...
static int x264_slices_write_thread_loop( x264_t *h )
{
	while( h->b_thread_active )
	{
		pthread_mutex_lock(&h->thread_mutex_start);
		while ( !h->b_thread_ready )
		{
			pthread_cond_wait(&h->thread_cond_start, &h->thread_mutex_start);
			if( !h->b_thread_active )
			{
	    		pthread_mutex_unlock(&h->thread_mutex_start);
				return 0;
			}
		}
	    pthread_mutex_unlock(&h->thread_mutex_start);
		...
		pthread_mutex_lock(&h->thread_mutex_stop);
		h->b_thread_finished = 1;
		pthread_cond_signal(&h->thread_cond_stop);
	    pthread_mutex_unlock(&h->thread_mutex_stop);
	}
	return 0;
}

// TID : SLICES. This function defines the number of MBs for a Slice 
void calcMBsSlice( x264_t *h, int idxSlice )
{
	h->sh.i_first_mb = idxSlice * (h->sps->i_mb_width * h->sps->i_mb_height) / h->param.i_slices;
	h->sh.i_last_mb = (idxSlice+1) * (h->sps->i_mb_width * h->sps->i_mb_height) / h->param.i_slices;
}
...
//      fprintf(stderr, "current: %p  prev: %p  oldest: %p \n", thread_current, thread_prev, thread_oldest);
    ...
    *pp_nal = NULL;
    ...
    if( h->frames.current[0] == NULL )
    {
        ...
        {
        	// TID : SLICES. Avoid Deadlock at the final frame of a stream
...
//    if( h->param.i_threads > 1 )
    ...
    else if( h->param.i_threads > 1 || h->param.i_slices > 1 )
    {
	    int t = h->param.i_threads;
    	
    	for( i = 0; i < (h->param.i_slices); i++ )
    	{
	        thread_current = ( i == 0 ) ? h : ((t > 1) ? h->thread[ i+t-1 ] : h->thread[ i ]);
	        thread_prev    = h;
	        thread_oldest  = h;
	        ...
	        {
	        	x264_thread_sync_context( thread_current, thread_prev );
	        	x264_thread_sync_ratecontrol( thread_current, thread_prev, NULL );
	    		/* Init the rate control */
    			x264_ratecontrol_start( thread_current, thread_current->fenc->i_qpplus1 );
	            /* Init bitstream context */
    			thread_current->out.i_nal = 0;
    			bs_init( &thread_current->out.bs, thread_current->out.p_bitstream, thread_current->out.i_bitstream );
    			/* Assign MB data to FrameDecode pointers (avoid unnecessary copies) */
    			x264_macroblock_slice_init( thread_current );
	        }
	        
    		calcMBsSlice(thread_current, i);
    		
			pthread_mutex_lock(&thread_current->thread_mutex_start);
			thread_current->b_thread_ready = 1;
	    	pthread_cond_signal(&thread_current->thread_cond_start);
	    	pthread_mutex_unlock(&thread_current->thread_mutex_start);
    	}
    }
    else
        x264_slices_write( h );
    ...
    //x264_encoder_frame_end( thread_oldest, thread_current, pp_nal, pi_nal, pic_out );
    x264_encoder_frame_end( thread_oldest, h, pp_nal, pi_nal, pic_out );
    return 0;
}
                                    ...
                                    x264_picture_t *pic_out )
{
    ...
    // TID : SLICES. Waiting for the threads
    if( h->param.i_threads > 1 && h->param.i_slices == 1 )
    {
	    if( h->b_thread_active )
	    {
    ...
    else if( h->param.i_threads > 1 || h->param.i_slices > 1 )
    {
	    ...
	    {
			pthread_mutex_lock(&h->thread_mutex_stop);
			while( !h->b_thread_finished )
			{
				pthread_cond_wait(&h->thread_cond_stop, &h->thread_mutex_stop);
			}
			pthread_mutex_unlock(&h->thread_mutex_stop);
			pthread_mutex_lock(&h->thread_mutex_stop);
			h->b_thread_finished = 0;
		    pthread_mutex_unlock(&h->thread_mutex_stop);
	    }
		for( i = 1 ; i < h->param.i_slices; i++ )
		{
			idxThread = (h->param.i_threads > 1) ? (i+h->param.i_threads-1) : i;
			if(h->thread[idxThread]->b_thread_active)
			{
				pthread_mutex_lock(&h->thread[idxThread]->thread_mutex_stop);
				while( !h->thread[idxThread]->b_thread_finished )
				{
					pthread_cond_wait(&h->thread[idxThread]->thread_cond_stop, &h->thread[idxThread]->thread_mutex_stop);
				}
				pthread_mutex_unlock(&h->thread[idxThread]->thread_mutex_stop);
				pthread_mutex_lock(&h->thread[idxThread]->thread_mutex_stop);
				h->thread[idxThread]->b_thread_finished = 0;
			    pthread_mutex_unlock(&h->thread[idxThread]->thread_mutex_stop);
			}
		    ...
		    for(j = 0; j < h->thread[idxThread]->out.i_nal; j++)
		    {
		    	h->out.nal[h->out.i_nal+j] = h->thread[idxThread]->out.nal[j];
		    }
			h->out.i_nal = h->out.i_nal + h->thread[idxThread]->out.i_nal;				
			
	    	h->out.i_frame_size += h->thread[idxThread]->out.i_frame_size;
	    	h->stat.frame.i_mb_count_i += h->thread[idxThread]->stat.frame.i_mb_count_i;
	        h->stat.frame.i_mb_count_p += h->thread[idxThread]->stat.frame.i_mb_count_p;
	        h->stat.frame.i_mb_count_skip += h->thread[idxThread]->stat.frame.i_mb_count_skip;
		}
...
//    for( i=0; i<h->param.i_threads; i++ )
    ...
    {
	    // TID : SLICES. Waiting for the threads
	    if( h->thread[i]->param.i_threads > 1 && h->thread[i]->param.i_slices == 1 )
	    {
	    ...
	    else
	    {
	        ...
	        {
	        	h->thread[i]->b_thread_active = 0;
				pthread_mutex_lock(&h->thread[i]->thread_mutex_start);
		    	pthread_cond_signal(&h->thread[i]->thread_cond_start);
		    	pthread_mutex_unlock(&h->thread[i]->thread_mutex_start);
				pthread_join(h->thread[i]->thread_handle, NULL);
	        
		        /* Clean up*/
		        pthread_mutex_destroy( &h->thread[i]->thread_mutex_start );
		  		pthread_cond_destroy( &h->thread[i]->thread_cond_start );
		        pthread_mutex_destroy( &h->thread[i]->thread_mutex_stop );
		  		pthread_cond_destroy( &h->thread[i]->thread_cond_stop );
	        }
	    }
    }
...
#define SUM3b(p,o) (p[SLICE_TYPE_I][o] + p[SLICE_TYPE_P][o] + p[SLICE_TYPE_B][o])
    ...
    x264_cqm_delete( h );
	// TID : SLICES. dealloc memory of the threads
