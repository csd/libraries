...
//	memset( rc, 0, h->param.i_threads * sizeof(x264_ratecontrol_t) ); 
    ...
    rc = h->rc = x264_malloc( num_threads * sizeof(x264_ratecontrol_t) );
    memset( rc, 0, num_threads * sizeof(x264_ratecontrol_t) );
...
void x264_ratecontrol_end( x264_t *h, int bits )
{
	...
	for( i = 1 ; i < h->param.i_slices; i++ )
	{
    	...
    	h->rc->qpa += h->thread[idxThread]->rc->qpa;

	}

    h->fdec->f_qp_avg = rc->qpa /= h->mb.i_mb_count;
