...
static void logError(char* formnat, ...) {}
...
int open_file_y4m( char *psz_filename, hnd_t *p_handle, x264_param_t *p_param )
{
    ...
    for( tokstart = &header[strlen(Y4M_MAGIC)+1]; tokstart < header_end; tokstart++ )
    {
        ...
        case 'I': /* Interlace type */
            ...
            default: interlaced = 1;
                logError("Warning, this sequence might be interlaced\n");
            }
            break;
        ...
        case 'X': /* Vendor extensions */
            if( !strncmp("YSCSS=",tokstart,6) )
            {
                ...
                {
                    logError("Unsupported extended colorspace\n");
                    return -1;
                }
            }
            tokstart = strchr(tokstart, 0x20);
    ...
    logError("yuv4mpeg: %ix%i@%i/%ifps, %i:%i\n",
...
int read_frame_y4m( x264_picture_t *p_pic, hnd_t handle, int i_frame )
{
    ...
    {
        logError("Bad header magic (%08X <=> %s)\n",
...
int open_file_avis( char *psz_filename, hnd_t *p_handle, x264_param_t *p_param )
{
    ...
    *p_handle = (hnd_t)h;
    ...
    {
        logError("avis [error]: unsupported input format (%c%c%c%c)\n",
    ...
    logError("avis [info]: %dx%d @ %.2f fps (%d frames)\n",
...
int set_param_mp4( hnd_t handle, x264_param_t *p_param )
{
    ...
    logError("mp4 [info]: initial delay %d (scale %d)\n", p_mp4->i_init_delay, p_mp4->i_time_res);
