...
#define H1 if(b_longhelp) printf
    ...
    H0( "\n" );
	// TID : SLICES. Help message. Specify number of Slices (Image divided by horizontal slices)
    H0( "      --slices <integer>	   Specify number of slices\n" );
    H0( "\n" );
}
...
#define OPT_LONGHELP 263
        ...
        static struct option long_options[] =
        {
			...
			// TID : SLICES. Number of Slices per Frame
        ...
        switch( c )
        {
            ...
            case 'o':
...
#else
                    logError("x264 [error]: not compiled with MP4 output support\n" );
                    return -1;
#endif
                }
                ...
                {
                    logError("x264 [error]: can't open output file `%s'\n", optarg );
                    return -1;
                }
                break;
            case OPT_QPFILE:
                ...
                if( !opt->qpfile )
                {
                    logError("x264 [error]: can't open `%s'\n", optarg );
                    return -1;
                }
            ...
            case OPT_VISUALIZE:
                ...
                b_exit_on_ctrl_c = 1;
#else
                logError("x264 [warning]: not compiled with visualization support\n" );
#endif
                break;
            default:
            {
        ...
        if( b_error )
        {
            ...
            logError("x264 [error]: invalid argument: %s = %s\n", name, optarg );
            return -1;
        }
    }
    ...
    {
        logError("x264 [error]: No %s file. Run x264 --help for a list of options.\n",
            ...
            for( psz = psz_filename; *psz; psz++ )
            {
                    ...
                    && sscanf( psz, "%ux%u", &param->i_width, &param->i_height ) == 2 )
                {
                    if( param->i_log_level >= X264_LOG_INFO )
                        logError("x264 [info]: file name gives %dx%d\n", param->i_width, param->i_height );
                    break;
                }
            }
        }
    ...
    {
        logError("x264 [error]: Rawyuv input requires a resolution.\n" );
        return -1;
    }
    ...
    /* open the input */
    {
...
#else
            logError("x264 [error]: not compiled with AVIS input support\n" );
            return -1;
#endif
        }
        if ( b_y4m )
        ...
        {
            logError("x264 [error]: could not open input file '%s'\n", psz_filename );
            return -1;
        }
    }
    ...
    if( b_thread_input || param->i_threads > 1 )
    {
        ...
        {
            logError("x264 [warning]: threaded input failed\n" );
        }
        else
        {
...
static void parse_qpfile( cli_opt_t *opt, x264_picture_t *pic, int i_frame )
{
    ...
    while( num < i_frame )
    {
        ...
        {
            logError("x264 [error]: can't parse qpfile for frame %d\n", i_frame );
            fclose( opt->qpfile );
            opt->qpfile = NULL;
...
static int  Encode_frame( x264_t *h, hnd_t hout, x264_picture_t *pic )
{
    ...
    {
        logError("x264 [error]: x264_encoder_encode failed\n" );
    }
    ...
    for( i = 0; i < i_nal; i++ )
    {
        ...
        {
            logError("x264 [error]: need to increase buffer size (size=%d)\n", -i_size );
        }
    }
    if (i_nal)
...
static int  Encode( x264_param_t *param, cli_opt_t *opt )
{
    ...
    {
        logError("x264 [error]: x264_encoder_open failed\n" );
    ...
    {
        logError("x264 [error]: can't set outfile param\n" );
        p_close_infile( opt->hin );
        p_close_outfile( opt->hout );
        return -1;
    }
         ...
         b_ctrl_c == 0 && (i_frame < i_frame_total || i_frame_total == 0); )
    {
                            ...
                            : i_frame % 10 == 0 ) )
        {
            ...
            if( i_frame_total )
            {
                ...
                logError("encoded frames: %d/%d (%.1f%%), %.2f fps, eta %d:%02d:%02d  \r",
                         i_frame, i_frame_total, (float)i_progress / 10, fps,
                         eta/3600, (eta/60)%60, eta%60 );
            }
            else
                logError("encoded frames: %d, %.2f fps   \r", i_frame, fps );
    ...
    if( b_ctrl_c )
        logError("aborted at input frame %d\n", opt->i_seek + i_frame );
    ...
    if( i_frame > 0 )
    {
        ...
        logError("encoded %d frames, %.2f fps, %.2f kb/s\n", i_frame, fps,
